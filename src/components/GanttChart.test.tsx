...
describe('GanttChart Component', () => {
  test('renders and initializes Gantt correctly', async () => {
    ...
    const ganttContainer = screen.getByRole('generic', { name: /gantt-chart-area/i }); // Assuming you add an aria-label or similar
    ...
    expect(screen.getByText(/Gantt Chart \(dhtmlx-gantt\)/i)).toBeInTheDocument();
    ...
    await waitFor(() => {
      ...
      expect(gantt.parse).toHaveBeenCalledWith(expect.objectContaining({
  ...
  test('applies configurations and templates', async () => {
    ...
    await waitFor(() => {
      ...
      expect(gantt.config.columns).toEqual(expect.arrayContaining([
    ...
    if (gantt.templates.timeline_cell_class) {
        ...
        (gantt.date.date_to_str("%Y-%m-%d") as jest.Mock).mockReturnValueOnce("2024-01-01");
    ...
    if (gantt.templates.task_class) {
        const mockTask = { id: 1, text: 'Test', start_date: '2024-01-01', urgency: 'urgent', difficulty: 'easy', type: gantt.config.types.TASK };
        (gantt.getState as jest.Mock).mockReturnValueOnce({ selected_task: null }); // ensure not selected for this part
        const taskClass = gantt.templates.task_class(new Date(), new Date(), mockTask);
